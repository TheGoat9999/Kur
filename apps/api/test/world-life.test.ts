...
test('planned outer-world streets stay valid map entities without becoming playable street scenes', () => {
  ...
  assert.equal(map.segments[0]?.playable, false);
  assert.equal(map.parcels[0]?.name, 'Mesa Roja General Store');
  assert.deepEqual(resolveWorldPath(map, 'mesa_main_street_street_1_segment_1'), map.current);
});
