...
describe('street spatial contract', () => {
  it('projects movement onto authored pedestrian edges instead of a free-form rectangle', () => {
    ...
    expect(resolveStreetNavigationTarget('market_block_3', { x: 79, y: 38 })?.position).toEqual({ x: 79, y: 42 });
    expect(clampStreetPosition('market_block_3', { x: 79, y: 38 })).toEqual({ x: 79, y: 42 });
  });

  it('allows continuous destinations along a sidewalk edge', () => {
    const route = getStreetRoute('market_block_3', { x: 50, y: 67 }, { x: 73.4, y: 65 });
  ...
  it('uses a stable navigation point after street travel', () => {
