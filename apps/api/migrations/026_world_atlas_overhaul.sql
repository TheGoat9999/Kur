...
WITH district_street_templates AS (
  ...
  SELECT 'atlas_' || district_id || '_' || n,district_id,street_name,street_kind,geometry,2,100 + n
...
SELECT id || '_segment',id,name || ' · Planned street',CASE WHEN kind='alley' THEN 'alley' ELSE 'road' END,geometry,false,100 + sort_order
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM world_districts d
