...
BEGIN
  SELECT patient_player_id INTO patient_id FROM ems_calls WHERE id = NEW.call_id;
  IF patient_id IS NULL THEN RETURN NEW; END IF;
  INSERT INTO player_needs_runtime(player_id) VALUES(patient_id) ON CONFLICT(player_id) DO NOTHING;
  UPDATE player_needs_runtime
    SET consciousness = CASE WHEN NEW.consciousness='unresponsive' THEN 'unconscious' ELSE 'conscious' END,
        pain = GREATEST(pain, LEAST(100, NEW.pain * 10)), updated_at=now()
    WHERE player_id=patient_id;
  bleed := CASE NEW.bleeding WHEN 'major' THEN 3 WHEN 'minor' THEN 1 ELSE 0 END;
  IF bleed > 0 THEN
    SELECT id INTO existing_id FROM player_injuries WHERE player_id=patient_id AND bleeding>0 ORDER BY bleeding DESC,created_at DESC LIMIT 1;
    IF existing_id IS NULL THEN
      INSERT INTO player_injuries(player_id,kind,body_area,severity,bleeding) VALUES(patient_id,'other','general',CASE WHEN bleed=3 THEN 3 ELSE 1 END,bleed);
    ELSE
      UPDATE player_injuries SET bleeding=GREATEST(bleeding,bleed),severity=GREATEST(severity,CASE WHEN bleed=3 THEN 3 ELSE 1 END),updated_at=now() WHERE id=existing_id;
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE OR REPLACE FUNCTION sync_ems_transport_to_needs() RETURNS trigger AS $$
BEGIN
  IF NEW.status='transporting' AND OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO player_needs_runtime(player_id,care_state) VALUES(NEW.patient_player_id,'transporting')
      ON CONFLICT(player_id) DO UPDATE SET care_state='transporting',updated_at=now();
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION sync_ems_handoff_to_needs() RETURNS trigger AS $$
BEGIN
  IF NEW.outcome='transported' THEN
    INSERT INTO player_needs_runtime(player_id,care_state,consciousness,pain,admitted_until)
      VALUES(NEW.patient_player_id,'admitted','conscious',0,now()+interval '30 minutes')
      ON CONFLICT(player_id) DO UPDATE SET care_state='admitted',consciousness='conscious',pain=GREATEST(0,player_needs_runtime.pain-35),admitted_until=now()+interval '30 minutes',updated_at=now();
  ...
  ELSE
    UPDATE player_needs_runtime SET care_state='field',updated_at=now() WHERE player_id=NEW.patient_player_id;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
