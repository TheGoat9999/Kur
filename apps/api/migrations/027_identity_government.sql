...
BEGIN
  ...
  ON CONFLICT (player_id) DO UPDATE
    SET character_id = COALESCE(EXCLUDED.character_id, citizen_identities.character_id), updated_at = now()
...
BEGIN
  ...
  IF NEW.license_stage >= 2 THEN
    ...
    IF NOT FOUND THEN
      ...
      VALUES(v_identity_id,'professional','real_estate_agent','real_estate',NEW.player_id::text,'{"authority":"Sol Dorado Real Estate Commission"}'::jsonb);
    END IF;
  ELSE
...
INSERT INTO government_licenses(citizen_identity_id,license_kind,license_code,source_system,source_ref,metadata)
SELECT ensure_citizen_identity_db(rec.player_id),'professional','real_estate_agent','real_estate',rec.player_id::text,'{"authority":"Sol Dorado Real Estate Commission"}'::jsonb
FROM real_estate_careers rec WHERE rec.license_stage>=2 AND NOT EXISTS (
  SELECT 1 FROM government_licenses gl WHERE gl.citizen_identity_id=ensure_citizen_identity_db(rec.player_id) AND gl.license_kind='professional' AND gl.license_code='real_estate_agent' AND gl.status='active');
...
BEGIN
  v_identity_id := ensure_citizen_identity_db(NEW.player_id);
  UPDATE government_licenses SET status='revoked',updated_at=now()
  WHERE citizen_identity_id=v_identity_id AND license_kind='professional' AND source_system='jobs' AND status='active' AND NOT (license_code=ANY(NEW.qualifications));
  FOREACH v_qualification IN ARRAY NEW.qualifications LOOP
    ...
    IF NOT FOUND THEN
      INSERT INTO government_licenses(citizen_identity_id,license_kind,license_code,source_system,source_ref,metadata)
      VALUES(v_identity_id,'professional',v_qualification,'jobs',NEW.player_id::text,'{"credentialType":"qualification"}'::jsonb);
    END IF;
  END LOOP;
  RETURN NEW;
END;
...
INSERT INTO government_licenses(citizen_identity_id,license_kind,license_code,source_system,source_ref,metadata)
SELECT ensure_citizen_identity_db(jp.player_id),'professional',q.qualification,'jobs',jp.player_id::text,'{"credentialType":"qualification"}'::jsonb
FROM job_profiles jp CROSS JOIN LATERAL unnest(jp.qualifications) AS q(qualification)
