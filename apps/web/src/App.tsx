...
export function App() {
  ...
  function changeScreen(next: Screen) {
    if (next === 'inventory') { setVehicleMapFocusId(null); setPhoneOpen(false); if (screen !== 'world') { setScreen('world'); setInventoryOpen(true); } else setInventoryOpen(value => !value); setMenuOpen(false); return; }
    if (next === 'vehicles') setVehicleMode('my'); setVehicleMapFocusId(null); setInventoryOpen(false); setPhoneOpen(false); setScreen(next);
  }
  ...
  return <Shell state={state} screen={screen} inventoryOpen={inventoryOpen} menuOpen={menuOpen} onScreen={changeScreen} onMenu={setMenuOpen}>
    ...
    {screen === 'world' && <div className="world-inventory-stage h-full min-h-0"><WorldView state={state} onStateChange={setState} focusVehicleId={vehicleMapFocusId} onVehicleFocusHandled={() => setVehicleMapFocusId(null)} />{!inventoryOpen && !phoneOpen && <PhoneLauncher onOpen={() => setPhoneOpen(true)} />}{inventoryOpen && <InventoryModalV05 onStateChange={setState} onClose={() => setInventoryOpen(false)} />}{phoneOpen && <PhoneOverlay state={state} onClose={() => setPhoneOpen(false)} onOpenFeature={openPhoneFeature} />}</div>}
