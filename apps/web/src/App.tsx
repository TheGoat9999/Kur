...
export function App() {
  ...
  function locateVehicle(vehicleId:string){setInventoryOpen(false);setPhoneOpen(false);setEmsWorkspaceOpen(false);setEmergencyOpen(false);setMenuOpen(false);setVehicleMapFocusId(vehicleId);setScreen('world');}
  function changeScreen(next:Screen){setEmsWorkspaceOpen(false);setEmergencyOpen(false);if(next==='inventory'){setVehicleMapFocusId(null);setPhoneOpen(false);if(screen!=='world'){setScreen('world');setInventoryOpen(true);}else setInventoryOpen(value=>!value);setMenuOpen(false);return;}setVehicleMapFocusId(null);setInventoryOpen(false);setPhoneOpen(false);setScreen(next);}
  function openPhoneFeature(feature:'finance'|'jobs'){setVehicleMapFocusId(null);setPhoneOpen(false);setInventoryOpen(false);setEmsWorkspaceOpen(false);setEmergencyOpen(false);setMenuOpen(false);setScreen(feature);}
  ...
  return <Shell state={state} screen={screen} inventoryOpen={inventoryOpen} menuOpen={menuOpen} onScreen={changeScreen} onMenu={setMenuOpen}>
    ...
    {screen==='jobs'&&<>
      <section className="mb-4 flex flex-col gap-3 rounded-2xl border border-cyan-300/15 bg-[linear-gradient(135deg,rgba(8,28,36,.92),rgba(8,17,22,.96))] p-4 shadow-lg md:flex-row md:items-center md:justify-between"><div><div className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-300">SOL DORADO MEDICAL</div><h2 className="mt-1 text-base font-black text-slate-50">{locale==='bg'?'EMS · Служебен достъп':'EMS · Staff access'}</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-400">{locale==='bg'?'Диспечерът, активните случаи и медицинският MDT са служебни инструменти и вече се отварят от секцията за работа и кариера, а не от публичния 112 бутон.':'Dispatch, active cases and the medical MDT are staff tools and now open from Jobs & Careers instead of the public 112 button.'}</p></div><button onClick={()=>setEmsWorkspaceOpen(true)} className="min-h-11 shrink-0 rounded-xl border border-cyan-300/25 bg-cyan-300/10 px-4 text-sm font-black text-cyan-100 hover:border-cyan-300/45">✚ {locale==='bg'?'Отвори EMS / MDT':'Open EMS / MDT'}</button></section>
      <JobsView onStateChange={setState}/>{emsWorkspaceOpen&&<EmsView onStateChange={setState} onClose={()=>setEmsWorkspaceOpen(false)}/>} 
    </>}
