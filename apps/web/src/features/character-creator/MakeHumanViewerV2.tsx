...
const EYE: Record<string, string> = {
  'dark-brown': '#3b251b', brown: '#6b4a32', hazel: '#806c43', amber: '#a16c2b',
  green: '#567052', blue: '#4a708d', gray: '#748084'
};
...
type AccessoryData = { spec: MakeHumanSystemAsset; meta: MHCLO; obj: AccessoryObj; material: AssetMaterial };
type ViewerSummary = { loadedAssets: number; texturedAssets: number; failedAssets: number };
...
export function MakeHumanViewer({ recipe, focus = 'full' }: { recipe: CharacterAppearanceRecipe; focus?: 'full' | 'face' }) {
  ...
  const [status, setStatus] = useState('Зареждане на героя…');
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        console.error('[character-v2] init failed', error);
        if (!cancelled) {
          setFailed(true);
  ...
  useEffect(() => {
    ...
    void runtime.rebuild(recipe).then(summary => {
      ...
      console.error('[character-v2] rebuild failed', error);
      setFailed(true);
  ...
  return <div className="relative h-full min-h-[440px] overflow-hidden rounded-2xl bg-[radial-gradient(circle_at_50%_18%,#29373d,#111a1f_55%,#0a1014)]">
    ...
    <div className={`absolute left-3 top-3 max-w-[84%] rounded-full border px-3 py-1 text-[10px] font-bold tracking-[.08em] backdrop-blur ${failed ? 'border-red-400/30 bg-red-950/60 text-red-100' : 'border-emerald-300/20 bg-black/45 text-emerald-200'}`}>{status}</div>
...
function summaryText(summary: ViewerSummary) {
  if (summary.failedAssets) return `HM08 V2 · нормални очи · ${summary.loadedAssets} елемента · ${summary.failedAssets} грешки`;
  if (summary.loadedAssets) return `HM08 V2 · нормални очи · ${summary.texturedAssets}/${summary.loadedAssets} текстурирани`;
  return 'HM08 V2 · нормални очи';
}
...
class ViewerRuntime {
  ...
  constructor(private THREE: any, private canvas: HTMLCanvasElement) {
    ...
    this.scene.add(new THREE.HemisphereLight(0xe6eef1, 0x253039, 1.35));
    const key = new THREE.DirectionalLight(0xffdfca, 2.7); key.position.set(3.8, 5.4, 5.2); this.scene.add(key);
    const fill = new THREE.DirectionalLight(0xc2ddeb, 1.15); fill.position.set(-4, 3.2, 2.5); this.scene.add(fill);
    const rim = new THREE.DirectionalLight(0xffcdb3, .75); rim.position.set(-2, 4, -4); this.scene.add(rim);

    const floor = new THREE.Mesh(new THREE.CircleGeometry(3.2, 64), new THREE.MeshStandardMaterial({ color: 0x222d32, roughness: .96 }));
  ...
  async rebuild(recipe: CharacterAppearanceRecipe): Promise<ViewerSummary> {
    const buildId = ++this.rebuildId;
    const base = await getBase();
    const targets = await weightedTargets(recipe);
    if (buildId !== this.rebuildId) return { loadedAssets: 0, texturedAssets: 0, failedAssets: 0 };

    const deformed = applyTargets(base, targets);
    const specs = requestedAssets(recipe);
    const loaded: AccessoryData[] = [];
    let failedAssets = 0;

    const settled = await Promise.allSettled(specs.map(spec => getAccessoryData(spec)));
    settled.forEach((result, index) => {
      if (result.status === 'fulfilled') loaded.push(result.value);
      else {
        failedAssets += 1;
        console.warn('[character-v2] asset load failed', specs[index]?.id, result.reason);
      }
    });

    if (buildId !== this.rebuildId) return { loadedAssets: 0, texturedAssets: 0, failedAssets };
    ...
    const bodyGeometry = buildBodyGeometry(this.THREE, base, deformed, hidden);
    this.clearSceneMeshes();

    const bodyMaterial = new this.THREE.MeshPhysicalMaterial({
    ...
    this.body = new this.THREE.Mesh(bodyGeometry, bodyMaterial);

    const bounds = measureBodyBounds(base, deformed);
    const heightScale = 1 + (recipe.appearance.height / 100) * .085;
    const worldScale = (2.02 * heightScale) / Math.max(bounds.maxY - bounds.minY, .001);
    const worldPosition = new this.THREE.Vector3(-bounds.centerX * worldScale, -bounds.minY * worldScale, -bounds.centerZ * worldScale);
    this.body.scale.setScalar(worldScale);
    this.body.position.copy(worldPosition);
    this.root.add(this.body);

    this.addEyes(base, deformed, worldScale, worldPosition, recipe.appearance.eyeColor);

    let texturedAssets = 0;
    for (const data of loaded) {
      if (buildId !== this.rebuildId) break;
      try {
        if (await this.addAccessory(data, deformed, worldScale, worldPosition, recipe)) texturedAssets += 1;
      } catch (error) {
        failedAssets += 1;
        console.warn('[character-v2] render failed', data.spec.id, error);
      }
    }

    return { loadedAssets: loaded.length, texturedAssets, failedAssets };
  }

  setFocus(focus: 'full' | 'face') {
    this.updateCamera(focus === 'face' ? 1.70 : 1.05, focus === 'face' ? 1.30 : 4.25);
  }

  private addEyes(base: BaseMesh, deformed: number[][], worldScale: number, worldPosition: any, eyeColor: string) {
    for (const tris of [base.eyeTris.left, base.eyeTris.right]) {
      const metrics = eyeMetrics(deformed, tris);
      if (!metrics) continue;

      const radius = metrics.radius;
      const geometry = new this.THREE.SphereGeometry(radius, 48, 32);
      const eyeTexture = makeEyeTexture(this.THREE, EYE[eyeColor] ?? EYE.brown);
      eyeTexture.anisotropy = Math.min(8, this.renderer.capabilities.getMaxAnisotropy?.() ?? 1);
      this.textures.add(eyeTexture);
      ...
      const eye = new this.THREE.Mesh(geometry, material);
      eye.position.set(metrics.centerX, metrics.centerY, metrics.centerZ - radius * .12);
      eye.scale.set(1, .98, .96);
      this.applyWorldTransform(eye, worldScale, worldPosition);
      this.addExtra(eye);
    }
  }

  private async addAccessory(data: AccessoryData, deformed: number[][], worldScale: number, worldPosition: any, recipe: CharacterAppearanceRecipe) {
    ...
    let diffuse: any = null;
    let normal: any = null;
    let ao: any = null;

    if (data.material.diffuseTexture) diffuse = await this.loadAssetTexture(data.spec, data.material.diffuseTexture, true);
    if (data.material.normalTexture) normal = await this.loadAssetTexture(data.spec, data.material.normalTexture, false);
    if (data.material.aoTexture) ao = await this.loadAssetTexture(data.spec, data.material.aoTexture, false);

    const isHair = data.spec.kind === 'hair';
    const color = isHair
      ? (HAIR[recipe.grooming.hairColor] ?? HAIR['dark-brown'])
      : diffuse ? '#ffffff' : data.spec.fallbackColor;
    ...
    const material = new this.THREE.MeshStandardMaterial({
      ...
      normalMap: normal,
      aoMap: ao,
      aoMapIntensity: ao ? .78 : 1,
      roughness: isHair ? .70 : .58,
      metalness: 0,
      transparent: isHair || data.material.transparent,
      alphaTest: isHair ? .14 : .02,
    ...
    this.applyWorldTransform(mesh, worldScale, worldPosition);
    this.addExtra(mesh);
    return Boolean(diffuse);
  }

  private loadAssetTexture(spec: MakeHumanSystemAsset, file: string, srgb: boolean): Promise<any | null> {
    const safeFile = file.split(/[\\/]/).pop() || '';
    if (!safeFile) return Promise.resolve(null);
    const key = `${spec.id}/${safeFile}/${srgb ? 's' : 'l'}`;
    const cached = this.textureCache.get(key);
    if (cached) return cached;

    const category = spec.directory.endsWith('/hair') ? 'hair' : 'clothes';
    const url = `${API_URL}/v1/assets/makehuman?category=${encodeURIComponent(category)}&id=${encodeURIComponent(spec.id)}&file=${encodeURIComponent(safeFile)}`;
    const promise = new Promise<any | null>(resolve => {
      const loader = new this.THREE.TextureLoader();
      loader.setCrossOrigin?.('anonymous');
      loader.load(url, (texture: any) => {
        if (srgb) texture.colorSpace = this.THREE.SRGBColorSpace;
  ...
  private applyWorldTransform(object: any, scale: number, position: any) {
  ...
  private bindControls() {
    ...
    const move = (event: PointerEvent) => {
      ...
      if (!previous) return;
      const oldPinch = this.pinchBase;
      this.pointerMap.set(event.pointerId, point(event));
      if (this.pointerMap.size === 1) {
        this.yaw += (event.clientX - previous.x) * .008;
        this.root.rotation.y = this.yaw;
      } else if (this.pointerMap.size === 2) {
        const points = [...this.pointerMap.values()];
        const distance = Math.hypot(points[0].x - points[1].x, points[0].y - points[1].y) || 1;
        if (oldPinch && oldPinch.distance > 2) {
          this.distance = Math.max(.85, Math.min(6, this.distance * oldPinch.distance / distance));
          this.updateCamera();
        }
        this.pinchBase = { distance };
      }
    };
    const up = (event: PointerEvent) => { this.pointerMap.delete(event.pointerId); updatePinch(); };
...
function makeEyeTexture(THREE: any, irisColor: string) {
  const canvas = document.createElement('canvas');
  canvas.width = 1024;
  canvas.height = 512;
  const ctx = canvas.getContext('2d')!;

  const sclera = ctx.createLinearGradient(0, 0, 0, canvas.height);
  sclera.addColorStop(0, '#ded7ce');
  sclera.addColorStop(.48, '#f3eee7');
  sclera.addColorStop(1, '#d8d0c8');
  ...
  const irisRadius = 78;
  const iris = ctx.createRadialGradient(cx, cy, 8, cx, cy, irisRadius);
  iris.addColorStop(0, '#0b0b0b');
  iris.addColorStop(.23, '#11100f');
  iris.addColorStop(.30, irisColor);
  iris.addColorStop(.66, irisColor);
  iris.addColorStop(.87, shadeHex(irisColor, -.35));
  iris.addColorStop(1, '#191514');
  ctx.beginPath();
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const inner = 28 + (i % 5) * 2;
    const outer = 67 - (i % 4) * 3;
    ctx.strokeStyle = i % 3 === 0 ? 'rgba(255,236,190,.13)' : 'rgba(25,13,9,.14)';
    ctx.lineWidth = 2;
    ctx.beginPath();
  ...
  const shine = ctx.createRadialGradient(cx - 24, cy - 28, 0, cx - 24, cy - 28, 18);
  shine.addColorStop(0, 'rgba(255,255,255,.72)');
  shine.addColorStop(1, 'rgba(255,255,255,0)');
  ctx.fillStyle = shine;
  ctx.fillRect(cx - 50, cy - 52, 60, 60);
...
function eyeMetrics(deformed: number[][], tris: Tri[]) {
  const used = new Set<number>();
  for (const tri of tris) for (const [vi] of tri) used.add(vi);
  if (!used.size) return null;
  let minX = Infinity, minY = Infinity, minZ = Infinity, maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
  for (const index of used) {
    const p = deformed[index];
    if (!p) continue;
  ...
  const radius = Math.max(.12, Math.min(.34, Math.min(sizeX, sizeY, Math.max(sizeZ, .001)) * .47));
  return {
    centerX: (minX + maxX) / 2,
    centerY: (minY + maxY) / 2,
    centerZ: (minZ + maxZ) / 2,
...
async function getTarget(path: string): Promise<Target> {
  ...
  if (!value) { value = fetchMakeHumanText(path).then(parseTarget); targetCache.set(path, value); }
  return value;
}
...
function parseBaseObj(text: string): BaseMesh {
  ...
  if (eyeTris.left.length < 40 || eyeTris.right.length < 40) throw new Error('HM08 позициите за очите липсват');
...
function buildBodyGeometry(THREE: any, base: BaseMesh, verts: number[][], hidden: Set<number>) {
...
function measureBodyBounds(base: BaseMesh, deformed: number[][]) {
  ...
  let minX = Infinity, minY = Infinity, minZ = Infinity, maxX = -Infinity, maxY = -Infinity, maxZ = -Infinity;
  for (const index of used) {
    const p = deformed[index]; if (!p) continue;
    minX = Math.min(minX, p[0]); maxX = Math.max(maxX, p[0]);
    minY = Math.min(minY, p[1]); maxY = Math.max(maxY, p[1]);
    minZ = Math.min(minZ, p[2]); maxZ = Math.max(maxZ, p[2]);
  }
...
async function getAccessoryData(spec: MakeHumanSystemAsset) {
  ...
  if (!value) { value = loadAccessoryData(spec); accessoryCache.set(spec.id, value); }
...
async function loadAccessoryData(spec: MakeHumanSystemAsset): Promise<AccessoryData> {
  ...
  const obj = parseAccessoryOBJ(await fetchAssetText(spec, meta.objFile || `${base}.obj`));
  if (obj.maxFaceVertex >= meta.mappings.length) throw new Error(`${spec.displayNameBg}: несъвместими MHCLO и OBJ данни`);
  let material: AssetMaterial = { diffuseTexture: '', normalTexture: '', aoTexture: '', opacity: 1, transparent: spec.kind === 'hair', backfaceCull: true };
  if (meta.materialFile) {
    try { material = parseMHMAT(await fetchAssetText(spec, meta.materialFile)); }
    catch (error) { console.warn('[character-v2] material fallback', spec.id, error); }
  }
...
function assetUrls(spec: MakeHumanSystemAsset, file: string) {
  ...
  const path = `${spec.directory}/${spec.id}/${cleanFile}`;
  return [
...
async function fetchAssetText(spec: MakeHumanSystemAsset, file: string) {
  ...
  for (const url of assetUrls(spec, file)) {
    try {
      ...
      if (head.includes('<html') || head.includes('<!doctype html')) throw new Error('HTML вместо елемент');
      if (head.includes('version https://git-lfs.github.com/spec/v1')) throw new Error('Git LFS указател вместо елемент');
      return text;
...
function parseMHCLO(text: string): MHCLO {
  ...
  let mode = '', rangeStart: number | null = null;
  for (const raw of text.split(/\r?\n/)) {
    const line = raw.trim();
    if (!line || line.startsWith('#')) continue;
    const parts = line.split(/\s+/), key = parts[0];
    if (key === 'obj_file') { output.objFile = parts.slice(1).join(' '); mode = ''; }
    else if (key === 'material') { output.materialFile = parts.slice(1).join(' '); mode = ''; }
    else if (key === 'x_scale' || key === 'y_scale' || key === 'z_scale') {
      const dimension = key[0] === 'x' ? 0 : key[0] === 'y' ? 1 : 2;
      output.scales[dimension] = [+parts[1], +parts[2], +parts[3]]; mode = '';
      ...
      if (parts.length === 1) output.mappings.push({ v: [+parts[0], 0, 0], w: [1, 0, 0], o: [0, 0, 0] });
        ...
        v: [+parts[0], +parts[1], +parts[2]], w: [+parts[3], +parts[4], +parts[5]], o: [+(parts[6] || 0), +(parts[7] || 0), +(parts[8] || 0)]
      });
    } else if (mode === 'delete') {
      let range = false;
      for (const token of parts) {
        if (token === '-') { range = true; continue; }
        const current = Number(token); if (!Number.isInteger(current)) continue;
        if (range && rangeStart !== null) { for (let index = rangeStart; index <= current; index++) output.deleteVerts.add(index); range = false; }
...
function parseMHMAT(text: string): AssetMaterial {
  const output: AssetMaterial = { diffuseTexture: '', normalTexture: '', aoTexture: '', opacity: 1, transparent: false, backfaceCull: true };
  for (const raw of text.split(/\r?\n/)) {
    const parts = raw.trim().split(/\s+/), key = parts[0];
    if (key === 'diffuseTexture') output.diffuseTexture = parts.slice(1).join(' ');
    else if (key === 'normalmapTexture') output.normalTexture = parts.slice(1).join(' ');
    else if (key === 'aomapTexture') output.aoTexture = parts.slice(1).join(' ');
...
function fitMHCLO(mappings: MHCLOMapping[], deformed: number[][], scales: Array<[number, number, number] | null>) {
  ...
  return mappings.map(mapping => {
    ...
    for (let index = 0; index < 3; index++) {
      const base = deformed[mapping.v[index]]; if (!base) continue;
      point[0] += base[0] * mapping.w[index]; point[1] += base[1] * mapping.w[index]; point[2] += base[2] * mapping.w[index];
    }
    point[0] += mapping.o[0] * scale[0]; point[1] += mapping.o[1] * scale[1]; point[2] += mapping.o[2] * scale[2];
    return point;
  });
}
...
function buildAccessoryGeometry(THREE: any, obj: AccessoryObj, fitted: number[][]) {
  ...
  for (const tri of obj.tris) {
    ...
    for (const [vi, ti] of tri) {
      ...
      if (output === undefined) {
        output = map.size; map.set(key, output);
        const p = fitted[vi], uv = obj.uvs[ti] ?? [0, 0];
        positions.push(p[0], p[1], p[2]); uvs.push(uv[0], uv[1]);
      }
      indices.push(output);
    }
  }
