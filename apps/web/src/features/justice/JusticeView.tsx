...
export function JusticeView() {
  ...
  return <section className="space-y-4">
    ...
    <div className="grid grid-cols-2 gap-2 md:grid-cols-6">
      <Metric label={bg ? 'Intake' : 'Intake'} value={justice.dashboard.intake}/><Metric label={bg ? 'Досъдебни' : 'Pretrial'} value={justice.dashboard.pretrial}/><Metric label={bg ? 'За съд' : 'Court queue'} value={justice.dashboard.awaitingCourt}/><Metric label={bg ? 'В затвора' : 'Jailed'} value={justice.dashboard.jailed}/><Metric label={bg ? 'Пробация' : 'Probation'} value={justice.dashboard.probation}/><Metric label={bg ? 'Записи' : 'Records'} value={justice.dashboard.totalRecords}/>
    </div>
    ...
    {tab === 'cases' && <div className="grid gap-4 lg:grid-cols-[.72fr_1.28fr]">
      <section className="rounded-2xl border border-white/10 bg-[#0a1116] p-3"><div className="space-y-2">{justice.cases.length ? justice.cases.map(item => <CaseButton key={item.id} item={item} active={activeCase?.id === item.id} onClick={() => setSelectedId(item.id)} />) : <Empty>{bg ? 'Няма дела.' : 'No cases.'}</Empty>}</div></section>
