...
import {
  acceptEmsCall, applyEmsTreatment, getEms, handoffEmsCall, reportEmsCall,
  saveEmsAssessment, setEmsDuty, updateEmsStatus
} from './ems-api';

type Tab = 'dispatch' | 'mdt' | '112';
  ...
  return <div className="absolute inset-2 z-50 overflow-hidden rounded-3xl border border-cyan-200/15 bg-[#071116]/[.985] shadow-[0_28px_90px_rgba(0,0,0,.7)] backdrop-blur-xl md:inset-5">
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex shrink-0 items-center gap-2 overflow-x-auto border-b border-white/8 bg-black/20 p-2.5">
        ...
        <TabButton active={tab==='112'} onClick={()=>setTab('112')}>{bg?'112 сигнал':'Call 112'}</TabButton>
      ...
      <main className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 md:p-5">
        ...
        {!ems?<div className="grid min-h-64 place-items-center text-sm text-slate-400">{bg?'Зареждане на EMS…':'Loading EMS…'}</div>:
          ...
          tab==='mdt'?<MdtView records={records} query={recordQuery} onQuery={setRecordQuery} bg={bg}/>:
          <ReportView bg={bg} busy={busy} onReport={(priority,type,summary)=>mutate(()=>reportEmsCall(priority,type,summary))}/>
        }
      </main>
    </div>
  </div>;
...
function DispatchView({ems,bg,busy,mutate}:{ems:EmsState;bg:boolean;busy:boolean;mutate:(work:()=>Promise<any>,refreshHud?:boolean)=>Promise<void>}) {
  ...
  return <div className="mx-auto max-w-5xl space-y-4">
    <section className="rounded-2xl border border-white/10 bg-[linear-gradient(135deg,rgba(9,28,36,.96),rgba(7,15,19,.96))] p-4 md:p-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><div className="text-[10px] font-black uppercase tracking-[.2em] text-cyan-300">{bg?'EMS DISPATCH':'EMS DISPATCH'}</div><h3 className="mt-1 text-xl font-black">{bg?'Активни повиквания':'Open calls'}</h3><p className="mt-1 text-sm text-slate-400">{bg?'Приеми един случай, излез към адреса и работи по него до handoff.':'Accept one case, respond physically and carry it through handoff.'}</p></div><div className="text-xs text-slate-400">{bg?'Печалба за смяната':'Shift earnings'} <b className="ml-1 text-slate-100">${(ems.profile.shiftEarningsCents/100).toFixed(2)}</b></div></div>
    </section>
    ...
    {ems.dispatch.length===0?<Empty>{bg?'Няма чакащи сигнали. 112 сигналите се появяват тук в реално време.':'No pending calls. 112 reports appear here in real time.'}</Empty>:
...
function ReportView({bg,busy,onReport}:{bg:boolean;busy:boolean;onReport:(p:EmsPriority,t:string,s:string)=>void}) { const [priority,setPriority]=useState<EmsPriority>('p3');const [type,setType]=useState('Медицински инцидент');const [summary,setSummary]=useState('Нужна е медицинска помощ.');return <div className="mx-auto max-w-xl rounded-2xl border border-red-300/20 bg-red-400/[.045] p-5"><div className="text-[10px] font-black uppercase tracking-[.2em] text-red-300">112</div><h3 className="mt-1 text-xl font-black">{bg?'Подай медицински сигнал':'Report a medical emergency'}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{bg?'Сигналът използва текущата ти authoritative world позиция. Друг EMS играч може да го приеме от Dispatch.':'The call uses your current authoritative world position. Another EMS player can accept it from Dispatch.'}</p><label className="mt-4 block text-xs text-slate-400">{bg?'Приоритет':'Priority'}<select value={priority} onChange={e=>setPriority(e.target.value as EmsPriority)} className="mt-1 min-h-11 w-full rounded-xl border border-white/10 bg-[#081116] px-3">{(['p1','p2','p3','p4'] as EmsPriority[]).map(p=><option key={p} value={p}>{bg?PRIORITY_COPY[p].bg:PRIORITY_COPY[p].en}</option>)}</select></label><label className="mt-3 block text-xs text-slate-400">{bg?'Тип инцидент':'Incident type'}<input value={type} onChange={e=>setType(e.target.value)} className="mt-1 min-h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm"/></label><label className="mt-3 block text-xs text-slate-400">{bg?'Описание':'Summary'}<textarea value={summary} onChange={e=>setSummary(e.target.value)} className="mt-1 min-h-28 w-full rounded-xl border border-white/10 bg-black/20 p-3 text-sm"/></label><button disabled={busy||type.trim().length<2||summary.trim().length<2} onClick={()=>onReport(priority,type,summary)} className="mt-4 min-h-12 w-full rounded-xl bg-red-300 px-4 font-black text-slate-950 disabled:opacity-35">{bg?'Изпрати към 112':'Send to dispatch'}</button></div> }
