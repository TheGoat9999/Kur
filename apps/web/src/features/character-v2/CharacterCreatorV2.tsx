...
import { catalogFor, catalogItem, type CharacterCatalogSlot } from './characterCatalog';
...
export function CharacterCreatorV2({ state, onStateChange }: { state: BootstrapState; onStateChange: (next: BootstrapState) => void }) {
  ...
  const [promptMode, setPromptMode] = useState<'full-body'|'portrait'>('full-body');
  const [copied, setCopied] = useState(false);
  const prompt = useMemo(() => buildCharacterPrompt(recipe, promptMode), [recipe, promptMode]);
    ...
    <section className="grid gap-4 2xl:grid-cols-[minmax(420px,.92fr)_minmax(600px,1.08fr)]">
      ...
      <div className="glass-panel overflow-hidden">
        <header className="border-b border-white/8 px-4 py-4 md:px-5">
          <div className="flex flex-wrap items-start gap-3">
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 max-w-2xl text-xs leading-5 text-slate-400">Новият creator е илюстративен и AI-first. Няма 3D дрехи, rigging или ръчно моделиране. Всички избори се пазят като рецепта и могат да се превърнат в постоянен AI образ.</p>
            </div>
        ...
        <div className="max-h-[calc(100dvh-312px)] min-h-[480px] overflow-y-auto p-4 md:p-5">
          {tab === 'identity' && <IdentityPanel recipe={recipe} name={displayName} onName={setDisplayName} onSex={setSex} onAppearance={setAppearance} />}
          ...
          {tab === 'ai' && <AiPanel prompt={prompt} mode={promptMode} onMode={setPromptMode} copied={copied} onCopy={copyPrompt} />}
        </div>
      </div>
    </section>
  );
...
function IdentityPanel({ recipe, name, onName, onSex, onAppearance }: {
  ...
  return <div className="space-y-6">
    <Section title="Самоличност" subtitle="Основата на героя. Всички следващи AI версии използват тези параметри като identity anchor.">
    ...
    <Section title="Телосложение" subtitle="Това е визуален descriptor за илюстрацията, а не 3D mesh deformation.">
...
function FacePanel({ recipe, onAppearance }: { recipe: CharacterVisualRecipeV2; onAppearance: <K extends keyof CharacterVisualRecipeV2['appearance']>(key: K, value: CharacterVisualRecipeV2['appearance'][K]) => void }) {
  return <div className="space-y-6">
    <Section title="Кожа и форма" subtitle="Поддържаме ясни, контролируеми характеристики, които AI може да пази между различни outfit версии.">
...
function ClothesPanel({ recipe, onGrooming }: PanelProps) {
  return <div className="space-y-6"><Section title="Горна част"><CatalogGrid slot="top" recipe={recipe} value={recipe.grooming.top} onSelect={v => onGrooming('top',v)} /></Section><Section title="Връхна дреха"><CatalogGrid slot="outerwear" recipe={recipe} value={recipe.grooming.outerwear} onSelect={v => onGrooming('outerwear',v)} /></Section><Section title="Долна част"><CatalogGrid slot="bottoms" recipe={recipe} value={recipe.grooming.bottoms} onSelect={v => onGrooming('bottoms',v)} /></Section><Section title="Обувки"><CatalogGrid slot="shoes" recipe={recipe} value={recipe.grooming.shoes} onSelect={v => onGrooming('shoes',v)} /></Section></div>;
}

function AccessoriesPanel({ recipe, onGrooming }: PanelProps) {
  return <div className="space-y-6"><Section title="Очила"><CatalogGrid slot="eyewear" recipe={recipe} value={recipe.grooming.eyewear} onSelect={v => onGrooming('eyewear',v)} /></Section><Section title="Шапки"><CatalogGrid slot="headwear" recipe={recipe} value={recipe.grooming.headwear} onSelect={v => onGrooming('headwear',v)} /></Section><Section title="Бижута"><CatalogGrid slot="jewelry" recipe={recipe} value={recipe.grooming.jewelry} onSelect={v => onGrooming('jewelry',v)} /></Section><Section title="Други"><CatalogGrid slot="accessory" recipe={recipe} value={recipe.grooming.accessory} onSelect={v => onGrooming('accessory',v)} /></Section></div>;
}

function StylePanel({ recipe, onGrooming }: PanelProps) {
  return <div className="space-y-6"><Section title="Визуална посока" subtitle="Тези тагове влияят на AI art direction, а не само на един конкретен outfit."><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{VIBES.map(([id,label]) => <Choice key={id} active={recipe.grooming.vibe === id} title={label} onClick={() => onGrooming('vibe',id)} />)}</div></Section><Section title="Акцентен цвят"><div className="flex flex-wrap gap-2">{ACCENTS.map(color => <button key={color} onClick={() => onGrooming('accentColor',color)} className={`h-11 w-11 rounded-xl border-2 ${recipe.grooming.accentColor === color ? 'border-white' : 'border-white/10'}`} style={{backgroundColor:color}} />)}</div></Section></div>;
}

function AiPanel({ prompt, mode, onMode, copied, onCopy }: { prompt: string; mode: 'full-body'|'portrait'; onMode: (v:'full-body'|'portrait') => void; copied: boolean; onCopy: () => void }) {
  return <div className="space-y-5"><div className="rounded-2xl border border-fuchsia-300/15 bg-[linear-gradient(135deg,rgba(244,114,182,.08),rgba(56,189,248,.05))] p-4"><div className="text-[9px] font-black uppercase tracking-[.18em] text-fuchsia-200">AI GENERATION BOUNDARY</div><h2 className="mt-2 text-base font-black text-white">Prompt-ът вече се строи автоматично</h2><p className="mt-2 text-xs leading-5 text-slate-400">Следващият integration pass ще изпрати точно този prompt плюс одобрения canonical render към image provider. Това пази лицето и пропорциите при смяна само на дрехи или коса.</p></div><div className="flex gap-2">{(['full-body','portrait'] as const).map(value => <button key={value} onClick={() => onMode(value)} className={`rounded-xl border px-3 py-2 text-xs font-bold ${mode === value ? 'border-amber-300/40 bg-amber-300/10 text-amber-100' : 'border-white/10 text-slate-400'}`}>{value === 'full-body' ? 'Цял ръст' : 'Портрет'}</button>)}</div><div className="rounded-2xl border border-white/10 bg-black/20 p-4"><div className="mb-3 flex items-center justify-between gap-3"><Small>Генериран prompt</Small><button onClick={onCopy} className="rounded-lg border border-white/10 px-3 py-1.5 text-[10px] font-bold text-slate-300 hover:text-white">{copied ? 'Копирано ✓' : 'Копирай'}</button></div><pre className="whitespace-pre-wrap font-sans text-[11px] leading-5 text-slate-400">{prompt}</pre></div><div className="rounded-xl border border-amber-300/15 bg-amber-300/[.04] p-3 text-[10px] leading-5 text-slate-400"><b className="text-amber-200">Следваща стъпка:</b> provider adapter + image storage + reference-image regeneration. UI и recipe вече не зависят от конкретен AI доставчик.</div></div>;
}
...
function CatalogGrid({ slot, recipe, value, onSelect }: { slot: CharacterCatalogSlot; recipe: CharacterVisualRecipeV2; value: string; onSelect: (id:string)=>void }) {
  const entries = catalogFor(slot, recipe.body);
  return <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{entries.map(entry => <button key={entry.id} onClick={() => onSelect(entry.id)} className={`overflow-hidden rounded-xl border text-left transition ${value === entry.id ? 'border-amber-300/50 bg-amber-300/[.08]' : 'border-white/8 bg-black/15 hover:border-white/15'}`}><div className="relative h-16 overflow-hidden" style={{background:`linear-gradient(135deg,${entry.primary},${entry.secondary})`}}><div className="absolute -right-3 -top-5 h-20 w-20 rounded-full border-[16px] border-white/10" /><div className="absolute bottom-2 left-2 rounded bg-black/25 px-1.5 py-1 text-[8px] font-black uppercase tracking-[.12em] text-white/70">{entry.renderKind}</div></div><div className="p-2.5"><b className="block text-[11px] text-slate-200">{entry.nameBg}</b><small className="mt-1 block truncate text-[8px] uppercase tracking-[.1em] text-slate-500">{entry.tags.join(' · ')}</small></div></button>)}</div>;
}
function Section({ title, subtitle, children }: { title:string; subtitle?:string; children:React.ReactNode }) { return <section><h2 className="text-sm font-black text-slate-100">{title}</h2>{subtitle && <p className="mt-1 text-xs leading-5 text-slate-500">{subtitle}</p>}<div className="mt-4 space-y-4">{children}</div></section>; }
function Small({ children }: { children:React.ReactNode }) { return <div className="text-[9px] font-black uppercase tracking-[.16em] text-slate-500">{children}</div>; }
