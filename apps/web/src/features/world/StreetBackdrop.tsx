...
function MarketBoulevard({ alerted }: { alerted: boolean }) {
  return <>
    ...
    <Tree x={90} y={264} /><Tree x={332} y={264} /><Tree x={742} y={264} /><Tree x={1102} y={264} />
    <Tree x={282} y={505} /><Tree x={815} y={505} /><Lamp x={214} y={276} /><Lamp x={754} y={276} /><Lamp x={1080} y={510} />
    <BusStop x={710} y={505} />
    <StreetSign x={650} y={280} a="MARKET ST" b="VESPUCCI" />
    <Continuation y={379} />
  </>;
}
...
function CypressParkCorner() {
  return <>
    ...
    <Building x={38} y={38} w={292} h={188} label="CYPRESS APARTMENTS" tone="cool" />
    <Park x={365} y={40} w={350} h={186} />
    <Building x={752} y={44} w={392} h={180} label="LOCAL SHOPS" tone="warm" />
    <Building x={28} y={534} w={250} h={140} label="TOWNHOUSES" tone="cool" />
    <Building x={307} y={536} w={242} h={138} label="BODEGA" tone="warm" />
    <Parking x={585} y={548} w={235} h={112} />
    <Building x={862} y={536} w={302} h={138} label="PALMAS OFFICES" tone="cool" />

    <Tree x={405} y={82} /><Tree x={492} y={136} /><Tree x={602} y={85} /><Tree x={676} y={154} />
    <Tree x={350} y={505} /><Tree x={1110} y={498} /><Bench x={522} y={186} /><Bench x={634} y={186} />
    <BusStop x={618} y={505} /><Lamp x={335} y={276} /><Lamp x={760} y={278} />
    <StreetSign x={762} y={280} a="CYPRESS AVE" b="PALM GROVE" />
    <Continuation y={384} />
  </>;
}
...
function MiraServiceYard() {
  return <>
    ...
    <Building x={24} y={46} w={270} h={174} label="WAREHOUSE A" tone="industrial" />
    <Building x={330} y={34} w={320} h={186} label="SERVICE BLOCK" tone="industrial" />
    <Building x={716} y={42} w={338} h={178} label="EL CAMINO · REAR" tone="warm" />
    <Building x={1070} y={58} w={105} h={162} label="UTIL" tone="industrial" />
    <Building x={18} y={534} w={258} h={140} label="STORAGE" tone="industrial" />
    <LoadingYard x={302} y={538} w={288} h={126} />
    <Parking x={625} y={546} w={250} h={116} industrial />
    <Building x={910} y={534} w={266} h={140} label="WAREHOUSE B" tone="warm" />

    <Dumpster x={260} y={500} /><Crates x={408} y={507} /><Crates x={455} y={511} /><UtilityBox x={650} y={252} />
    <Lamp x={326} y={276} /><Lamp x={865} y={278} /><Lamp x={615} y={514} />
    <StreetSign x={770} y={280} a="MIRA LANE" b="SERVICE" />
    <Continuation y={398} industrial />
  </>;
}
...
function Building({ x, y, w, h, label, tone }: { x: number; y: number; w: number; h: number; label: string; tone: 'cool' | 'warm' | 'industrial' | 'alert' }) {
  ...
  return <g filter="url(#sd-shadow)">
    <rect x={x} y={y} width={w} height={h} rx="8" fill={fill} />
    <rect x={x + 12} y={y + 12} width={Math.max(20, w - 24)} height={Math.max(20, h - 24)} rx="5" fill="#203237" opacity=".62" />
    {Array.from({ length: Math.max(1, Math.floor(w / 72)) }, (_, index) => <rect key={index} x={x + 24 + index * 58} y={y + 38} width="32" height="35" rx="3" fill="url(#sd-glass)" opacity={tone === 'industrial' ? .44 : .72} />)}
    <rect x={x + w * .58 - 22} y={y + h - 61} width="44" height="61" rx="3" fill="#274249" stroke="#d9eff0" strokeOpacity=".16" />
    <text x={x + 18} y={y + h - 18} fill="#e8eeea" fontSize="11" fontWeight="800" letterSpacing="1.35">{label}</text>
  </g>;
}

function Park({ x, y, w, h }: { x: number; y: number; w: number; h: number }) {
  return <g filter="url(#sd-shadow)">
    <rect x={x} y={y} width={w} height={h} rx="28" fill="url(#sd-grass)" stroke="#78936c" strokeWidth="3" />
    <path d={`M${x + 25} ${y + h - 28}C${x + w * .34} ${y + 56},${x + w * .64} ${y + h - 44},${x + w - 28} ${y + 32}`} fill="none" stroke="#ccb990" strokeWidth="14" opacity=".65" />
    <ellipse cx={x + w * .48} cy={y + h * .53} rx={w * .13} ry={h * .16} fill="#2f7280" stroke="#71a0a0" strokeWidth="3" />
  </g>;
}

function Parking({ x, y, w, h, industrial = false }: { x: number; y: number; w: number; h: number; industrial?: boolean }) {
  return <g>
    <rect x={x} y={y} width={w} height={h} rx="5" fill={industrial ? '#3b4140' : '#505a5c'} stroke="#798284" strokeWidth="2" />
    {Array.from({ length: Math.max(2, Math.floor(w / 45)) }, (_, i) => <path key={i} d={`M${x + 15 + i * 42} ${y + 8}v${h - 16}`} stroke="#d6d0bc" strokeWidth="2" opacity=".34" />)}
  </g>;
}

function LoadingYard({ x, y, w, h }: { x: number; y: number; w: number; h: number }) {
  return <g><rect x={x} y={y} width={w} height={h} rx="4" fill="#464b48" stroke="#74766d" strokeWidth="2" />{Array.from({ length: 7 }, (_, i) => <path key={i} d={`M${x + i * 48} ${y + h}l70 -${h}`} stroke="#bea668" strokeWidth="4" opacity=".18" />)}</g>;
}

function SideStreet({ x, direction, width }: { x: number; direction: 'north' | 'south'; width: number }) {
  const y = direction === 'north' ? 205 : 448;
  const h = direction === 'north' ? 168 : 165;
  return <g><rect x={x} y={y} width={width} height={h} fill="#2d393d" /><path d={`M${x + width / 2} ${y}v${h}`} stroke="#cfb666" strokeWidth="3" strokeDasharray="20 18" opacity=".52" /></g>;
}

function Crosswalk({ x, y, h, muted = false }: { x: number; y: number; h: number; muted?: boolean }) {
  return <g opacity={muted ? .45 : .78}>{Array.from({ length: 8 }, (_, i) => <rect key={i} x={x} y={y + i * (h / 8)} width="104" height={h / 16} rx="2" fill="#e6e1d5" />)}</g>;
}

function MedianPalms() {
  return <g>{[120, 300, 760, 1040].map(x => <g key={x}><circle cx={x} cy="379" r="12" fill="#455942" /><path d={`M${x} 379v-18`} stroke="#8c7051" strokeWidth="4" /><path d={`M${x} 361l-13 -7m13 7l13 -7m-13 7l-4 -13m4 13l5 -13`} stroke="#4f8156" strokeWidth="5" strokeLinecap="round" /></g>)}</g>;
}
...
function StreetSign({ x, y, a, b }: { x: number; y: number; a: string; b: string }) {
  return <g><path d={`M${x} ${y}v45`} stroke="#273436" strokeWidth="6" /><rect x={x - 55} y={y - 14} width="110" height="22" rx="4" fill="#204f48" stroke="#7e9b8e" strokeWidth="2" /><rect x={x - 42} y={y + 11} width="84" height="20" rx="4" fill="#334b66" stroke="#8193a7" strokeWidth="2" /><text x={x} y={y + 1} textAnchor="middle" fill="#edf2e9" fontSize="9" fontWeight="800">{a}</text><text x={x} y={y + 25} textAnchor="middle" fill="#edf2e9" fontSize="8" fontWeight="800">{b}</text></g>;
}

function Continuation({ y, industrial = false }: { y: number; industrial?: boolean }) {
  return <g opacity=".74"><path d={`M20 ${y - 24}L2 ${y}l18 24M58 ${y - 24}L40 ${y}l18 24M1180 ${y - 24}l18 24-18 24M1142 ${y - 24}l18 24-18 24`} fill="none" stroke={industrial ? '#c0a35e' : '#e3c66f'} strokeWidth="7" strokeLinecap="round" strokeLinejoin="round" /></g>;
}
