...
export function InteractionPanel({ object, actionStates, busy, onAction, onClose }: {
  ...
  const { t } = useI18n();
  return (
    <aside className="street-interaction-panel" aria-label={t('world.contextualActions')}>
      <header>
        <span className={`street-interaction-symbol street-interaction-${object.kind}`}><GameIcon name={object.icon} size={19} /></span>
      ...
      <div className="street-action-list">
        {object.actions.map(actionId => {
          ...
          const disabled = busy !== null || availability !== 'available';
          return (
            <button
              type="button"
            ...
            >
              ...
              <span className="street-action-copy">
                ...
                <small>{availability === 'available' ? t(presentation.descriptionKey) : availabilityText(availability, state?.cooldownEndsAt ?? null, t)}</small>
              </span>
              <GameIcon name={availability === 'available' ? 'arrow-right' : 'lock'} size={14} />
            </button>
          );
        })}
      </div>
...
function availabilityText(
  availability: StreetActionState['availability'],
