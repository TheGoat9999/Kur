...
export function StreetObject({ definition, selected, alerted, onSelect }: {
  ...
  const exitSide = definition.kind === 'exit'
    ? definition.x < 50 ? 'street-object-exit-left' : 'street-object-exit-right'
    : '';

  return (
    <button
      type="button"
      className={`street-object street-object-${definition.kind} street-object-${definition.importance} ${exitSide} ${selected ? 'street-object-selected' : ''} ${alerted ? 'street-object-alerted' : ''}`}
      style={style}
