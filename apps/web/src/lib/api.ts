...
export async function saveCharacter(displayName: string, recipe: unknown): Promise<BootstrapState> {
  const response = await authenticatedFetch('/v1/character', { method: 'PUT', body: JSON.stringify({ displayName, recipe }) });
...
export async function travelWorldMap(segmentId: string): Promise<WorldMapTravelResult> {
  const response = await authenticatedFetch('/v1/world/map/travel', { method: 'POST', body: JSON.stringify({ segmentId }) });
...
export async function moveStreetPlayer(position: StreetPosition): Promise<StreetPositionResult> {
  const response = await authenticatedFetch('/v1/world/move', { method: 'POST', body: JSON.stringify(position) });
...
export async function runWorldAction(actionId: WorldActionId, expectedVersion: number): Promise<WorldActionResult> {
  const response = await authenticatedFetch('/v1/world/actions', { method: 'POST', body: JSON.stringify({ requestId: crypto.randomUUID(), actionId, expectedVersion }) });
  if (!response.ok) {
...
export async function moveInventoryItem(itemId: string, toContainerKey: InventoryContainerKey, toSlotIndex?: number): Promise<InventoryState> {
  ...
  if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; throw new Error(payload?.error ?? `Inventory move failed (${response.status})`); }
  return InventoryStateSchema.parse(await response.json());
}
...
export async function splitInventoryItem(itemId: string, quantity: number, toSlotIndex?: number): Promise<InventoryState> {
  ...
  if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; throw new Error(payload?.error ?? `Inventory split failed (${response.status})`); }
  return InventoryStateSchema.parse(await response.json());
}
...
export async function useInventoryItem(itemId: string): Promise<InventoryMutationResult> {
  ...
  if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; throw new Error(payload?.error ?? `Inventory use failed (${response.status})`); }
...
export async function readPhoneNotification(notificationId?: string, all = false): Promise<PhoneState> {
  const response = await authenticatedFetch('/v1/phone/notifications/read', { method: 'POST', body: JSON.stringify(notificationId ? { notificationId } : { all }) });
...
export async function savePhoneNote(input: { noteId?: string; title: string; body: string; pinned?: boolean }): Promise<PhoneState> {
  const response = await authenticatedFetch('/v1/phone/notes', { method: 'POST', body: JSON.stringify({ ...input, pinned: input.pinned ?? false }) });
